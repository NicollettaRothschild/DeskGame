...
function checkValid() {
    ...
    if (script.useCollider) {
        if (script.collider) {
            ...
            print("ChainController, Warning, Please set collider sphere sceneObject");
            return false;
        }
    }
    return true;
...
function initialize() {
    for (var i = 0; i < script.joints.length; i++) {
        ...
        if (i == 0 || i == script.joints.length - 1 && script.anchorCount == 2) {
            p = new global.Point(0.0, pos);//creating static particle for both start and end
            points.push(p);
        } else {
...
function updatePhysics(dt, iteration) {
    ...
    points[firstIndex].setPosition(global.MathLib.vec3.fromEngine(links[0].transform.getWorldPosition()))
    ...
    if (script.useCollider) {
        var colliderPos = global.MathLib.vec3.fromEngine(script.collider.getTransform().getWorldPosition());
